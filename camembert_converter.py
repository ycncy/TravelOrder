...
def tokenize_and_align_labels(sentences, entities, tokenizer):
    ...
    tokenized_inputs = tokenizer(
        ...
        padding=True,   # Ensures that all sentences in a batch have the same length, which is necessary for efficient processing in the model
        return_offsets_mapping=True # Returns the character start and end offsets of each token in offset_mapping, for aligning character-level labels to tokens.
    )
